...
The web package must be served over `http(s)`, not opened straight off the desktop as a `file://` path — browsers block a `file://` page from loading the activity's JSON. Upload it to any host above (or run a local static server) and it works.
