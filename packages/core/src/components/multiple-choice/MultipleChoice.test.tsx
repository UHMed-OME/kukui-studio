...
describe("MultipleChoice", () => {
  it("renders title, question and answers", () => {
  ...
  it("handles multi-correct selection with checkbox semantics", async () => {
    const user = userEvent.setup();
    const onSubmit = vi.fn();
    render(<MultipleChoice config={cfgMulti} onSubmit={onSubmit} />);
    expect(screen.getAllByRole("checkbox")).toHaveLength(4);
    await user.click(screen.getByRole("checkbox", { name: /co2/i }));
    await user.click(screen.getByRole("checkbox", { name: /^h2o/i }));
    await user.click(screen.getByRole("button", { name: /check/i }));
  ...
  it("aria-label on each option includes the state", () => {
    render(<MultipleChoice config={cfgSingle} onSubmit={vi.fn()} />);
    const radio = screen.getByRole("radio", { name: /carbon dioxide, not selected/i });
